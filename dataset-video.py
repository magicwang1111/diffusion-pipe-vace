...
INPUT_DIR = Path("/mnt/data/wangxi/diffusion-pipe/dataset/20250417chi/")
OUTPUT_DIR = Path("/mnt/data/wangxi/diffusion-pipe/dataset/20250417chi处理后v4")
BUCKETS = [97]
...
def has_text(video: Path) -> bool:
    ...
    try:
        ...
        # 使用 SAMPLE_COUNT 计算采样间隔
        #sample_interval = max(1, total // SAMPLE_COUNT)
        #frame_indices   = range(0, total, sample_interval)
        # 直接把 SAMPLE_COUNT 当作步长
        frame_indices   = range(0, total, SAMPLE_COUNT)
        frames_with_text = 0
...
def process_video(video: Path, out: Path):
    ...
    try:
        ...
        if total <= 0:
            ...
            cap.release()
            return
        bucket = select_bucket(total, BUCKETS)
        idxs = set(evenly_spaced_indices(total, bucket))
        ...
        if vw:
            ...
            if not check_cap.isOpened() or check_cap.get(cv2.CAP_PROP_FRAME_COUNT) == 0:
                logger.warning(f"  [process_video] output file verification failed, removing: {out}")
